...
class Model:
    ...
    def setupCNN(self):
        ...
        for i in range(numLayers):
            ...
            pool = tf.nn.max_pool2d(input=relu, ksize=(1, poolVals[i][0], poolVals[i][1], 1), strides=(
                1, strideVals[i][0], strideVals[i][1], 1), padding='VALID')
    ...
    def setupCTC(self):
        ...
        if self.decoderType == DecoderType.BestPath:
            ...
            wordChars = open('modelx/wordCharList.txt').read().splitlines()[0]
            corpus = open('datax/corpus.txt').read()

            # decode using the "Words" mode of word beam search
            self.decoder = word_beam_search_module.word_beam_search(tf.nn.softmax(
                self.ctcIn3dTBC, axis=2), 50, 'Words', 0.0, corpus.encode('utf8'), chars.encode('utf8'), wordChars.encode('utf8'))
    ...
    def save(self):
        "save model to file"
        self.snapID += 1
        self.saver.save(self.sess, 'modelx/snapshot', global_step=self.snapID)
